...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval,FM_pop] = deopt(fname,S_struct,u0)
...
if ~isempty(u0)
    FM_pop(1:min(size(u0,1),size(FM_pop,1)),:)= ...
        u0(1:min(size(u0,1),size(FM_pop,1)),:);
end
...
[fitness, constraints]= feval(fname,FM_pop(I_best_index,:));

S_val(1).I_nc= size(constraints,2);
S_val(1).FVr_ca= constraints;
S_val(1).I_no= 1;
S_val(1).FVr_oa(1)= fitness;
...
for k=2:I_NP                          % check the remaining members
  ...
  [fitness, constraints]= feval(fname,FM_pop(k,:));
  
  S_val(k).I_nc= size(constraints,2);
  S_val(k).FVr_ca= constraints;
  S_val(k).I_no= 1;
  S_val(k).FVr_oa(1)= fitness;
...
%-----Select which vectors are allowed to enter the new population------------
  for k=1:I_NP
      ...
      [fitness, constraints]= feval(fname,FM_ui(k,:));
  
      S_tempval.I_nc= size(constraints,2);
      S_tempval.FVr_ca= constraints;
      S_tempval.I_no= 1;
      S_tempval.FVr_oa(1)= fitness;
