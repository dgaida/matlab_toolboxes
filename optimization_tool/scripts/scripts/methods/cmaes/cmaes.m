...
% cmaes.m, Version 3.40.beta, last change: October, 2009 
...
%             * (repmat(diagC,1,mu) .* arz(:,fitness.idxsel(1:mu)).^2 * weights);
      ...
      % "active" CMA update: negative update, first to control pos. definiteness 
      if flgActiveCMA > 0
        neg.mu = ceil(min(4*N,lambda)/4);  % i.e. neg.mu <= N 
        artmp = arz(:,fitness.idxsel(lambda:-1:lambda - neg.mu + 1));
        artmp = artmp ./ repmat(sqrt(sum(artmp.^2, 1)), N, 1);
        neg.ccov = 1/20;  % 1/N is introduced by normalization with ||artmp(:,i)||==1
                          % 1/4 usually still works OK and without correction for pos. def.
                          % from 1/20 to 1/15 still the learning performance increases almost linearly
        if flgActiveCMA == 1  % check pos. definiteness, assuming BD*BD' == C 
          % eigsopts.disp = 0;
          % maxeigenval = eigs(artmp * artmp', 1, 'lm', eigsopts);  % not faster
          maxeigenval = max(eig(artmp * artmp'));
          if maxeigenval >= 1/neg.ccov/2
            disp(['active CMA at iteration ' num2str(countiter) ': max EV ==', num2str(maxeigenval)]);
          end
          % ensure positive definiteness, if BD*BD' >= 0.49 * C in all directions (is always the case)
          neg.ccov = min(neg.ccov, 0.49/maxeigenval); % remove less than 49% of variance in any direction
        end
        artmp = BD * artmp;
        C = (1 + neg.ccov*neg.mu/N) * C - neg.ccov * artmp*artmp';
        % save
      end
...
%  f = fsum .* exp(0.1*randn(1,popsi));
  f = fsum .* (1 + (10/(N+10))*randn(1,popsi));
