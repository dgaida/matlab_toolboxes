%% arrow3d
% Plotting a 3-dimensional arrow
%
function [h,X1,Y1,Z1]= arrow3d(x,y,z,head_frac,radii,radii2,colr, varargin)
%% Release: 1.1
...
if (draw)
  ...
  % color given as char or as 3d vector
  if ischar(colr) || numel(colr) ~= numel(X1)
    h=surf(X1,Y1,Z1,'facecolor',colr,'edgecolor','none');
    camlight
    lighting phong
  else
    % color should be given as is X1
    h=surf(X1,Y1,Z1, colr,'facecolor','interp', ...
                    'edgecolor','none', ...
                    ...'FaceAlpha','opaque', ...
                    'AlphaDataMapping', 'direct', ...
                    'AlphaData',0.5.*ones(size(X1)));
  end
